...
def weather_Data():
    ...
    URL = f"https://api.openweathermap.org/data/2.5/weather?q={city}&lang=fa&units=metric&appid={API_Key}"

    if city != "" or city != " ":
        responce = requests.get(URL)
        j_Req = responce.json()

        sky_Description_D = j_Req["weather"][0]["description"]

        temp_D = j_Req["main"]["temp"]
        temp_D_U = f"{temp_D} °C"

        humidity_D = j_Req["main"]["humidity"]
        humidity_D_P = f"{humidity_D}%"

        wind_Speed_D = j_Req["wind"]["speed"]
